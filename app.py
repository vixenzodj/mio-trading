...
selected_dte = st.sidebar.multiselect("SCADENZE 0DTE/1DTE", date_options, default=[date_options[0]])
...
gran = st.sidebar.select_slider("GRANULARITÀ", options=[1, 2, 5, 10, 20, 25, 50, 100, 250], value=max(min_safe_gran, 10 if spot > 5000 else 5))
...
if selected_dte:
    ...
    if not raw_data.empty:
        ...
        # --- AGGIUNTA LOGICA DEVIAZIONI STANDARD ---
        mean_iv = raw_data['impliedVolatility'].mean()
        # Calcoliamo il movimento atteso basato sulla IV media
        dte_val = (datetime.strptime(target_dates[0], '%Y-%m-%d') - today).days + 0.5
        sd_move = spot * mean_iv * np.sqrt(max(dte_val, 1)/365)
        ...
        # --- LINEE DI LIVELLO (INCLUSE DEVIAZIONI STANDARD) ---
        ...
        # Deviazioni Standard: 1SD (Arancio) e 2SD (Viola)
        fig.add_hline(y=sd1_up, line_color="#FF851B", line_dash="dash", annotation_text="1SD UP")
        fig.add_hline(y=sd1_down, line_color="#FF851B", line_dash="dash", annotation_text="1SD DOWN")
        fig.add_hline(y=sd2_up, line_color="#B10DC9", line_dash="dashdot", annotation_text="2SD EXTREME")
        fig.add_hline(y=sd2_down, line_color="#B10DC9", line_dash="dashdot", annotation_text="2SD EXTREME")
        ...
        st.code(f"Pivots: 0G@{z_gamma:.2f} | 1SD: ±{sd_move:.2f} | 2SD: ±{sd_move*2:.2f}")
