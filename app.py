...
# --- CONFIGURAZIONE ---
st.set_page_config(layout="wide", page_title="GEX PRO TERMINAL V17", initial_sidebar_state="expanded")
...
def calc_greeks_pro(row, spot, t_yrs, r=0.045):
    try:
        ...
        # Dollar Gamma: Lo standard istituzionale (Spot^2 * Gamma * 0.01)
...
# --- SIDEBAR ---
st.sidebar.header("🕹️ GEX ENGINE V17")
...
if t_str:
    ...
    num_levels = st.sidebar.slider("VISUAL RANGE", 20, 200, 80)
    ...
    try:
        ...
        if not hist.empty:
            ...
            # Unificazione senza duplicati
            df_total = pd.concat([df_c, df_p]).groupby('strike', as_index=False).sum()
            df_total = df_total.sort_values('strike')

            # --- LOGICA ZERO FLIP & WALLS ---
            # Gamma Cumulativo per trovare il punto di equilibrio totale (Gexbot style)
            df_total['cum_gamma'] = df_total['Gamma'].cumsum()
            zero_idx = (df_total['cum_gamma']).abs().idxmin()
            z_gamma = df_total.loc[zero_idx, 'strike']
            
            # Filtro locale per i Muri Operativi (±10% dallo Spot)
            ...
            # --- DASHBOARD ---
            st.markdown(f"## 🏛️ {active_t} Dashboard | Spot: {spot:.2f}")
            
            # Bias istituzionale: Somma pesata delle pressioni
            bias_val = df_total['Gamma'].sum() + (df_total['Vanna'].sum() * 0.5)
            b_label, b_color = ("LONG GAMMA (Stabile)", "#00ff00") if bias_val > 0 else ("SHORT GAMMA (Volatile)", "#ff4444")
            ...
            d2.metric("CALL WALL", f"{call_wall:.0f}", f"{call_wall-spot:.1f} pts")
            d3.metric("PUT WALL", f"{put_wall:.0f}", f"{put_wall-spot:.1f} pts")
            d4.metric("ZERO FLIP", f"{z_gamma:.0f}", "VOL TRIGGER")

            # --- GRAFICO PROFILO ---
            # CORREZIONE: Uso di np.floor per evitare l'errore 'Series object has no attribute floor'
            ...
            fig.update_layout(
                ...
                title=f"Analisi di Esposizione Istituzionale - {active_t}"
            )
