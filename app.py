...
def calculate_gex_at_price(price, df, r=0.045):
    """Calcola l'esposizione Gamma netta dei dealer."""
    K = df['strike'].values
    iv = df['impliedVolatility'].values
    T = np.maximum(df['dte_years'].values, 0.0001)
    # OI + 50% Volume come proxy esposizione
...
def get_greeks_pro(df, S, r=0.045):
    """Calcola le Greche (Gamma, Vanna, Charm, Vega, Theta)."""
...
if menu == "🏟️ DASHBOARD SINGOLA":
    ...
    if h.empty: 
        st.error("Errore nel caricamento dati ticker.")
        st.stop()
    spot = h['Close'].iloc[-1]
    ...
    selected_dte = st.sidebar.multiselect("SCADENZE (0-50 DTE)", short_term_dates, default=short_term_dates[:5])
    metric = st.sidebar.radio("METRICA", ["Gamma", "Vanna", "Charm", "Vega", "Theta"])
    
    # GRANULARITÀ ATTIVATA NEL CODICE SOTTO
    gran = st.sidebar.select_slider("GRANULARITÀ STRIKE", options=[1, 2, 5, 10, 20, 25, 50], value=1) 
    ...
    if selected_dte:
        ...
        if not raw_data.empty:
            ...
            # --- LOGICA DI GRANULARITÀ (BINNING) ---
            # Questo raggruppa gli strike vicini in base alla tua scelta
            df['strike'] = (np.round(df['strike'] / gran) * gran)
            ...
            # --- GRAFICO ---
            p_df = visible_agg.copy()
            fig = go.Figure()
            fig.add_trace(go.Bar(y=p_df['strike'], x=p_df[metric], orientation='h',
                                 marker=dict(color=['#00FF41' if x >= 0 else '#FF4136' for x in p_df[metric]], line_width=0),
                                 width=gran * 0.8)) 
            ...
            fig.update_layout(template="plotly_dark", height=800, margin=dict(l=0,r=0,t=0,b=0),
                              yaxis=dict(range=[lo, hi], gridcolor="#333", title="Strike Price", dtick=gran),
                              xaxis=dict(title=f"Net {metric}", tickformat="$.3s"))
            
            st.plotly_chart(fig, use_container_width=True)

# =================================================================
# PAGINA 2: SCANNER 50 TICKER
# =================================================================
    ...
    for i, t_name in enumerate(tickers_50):
        ...
        try:
            ...
            # Label di analisi
            if px < zg_val:
                status_label = "🔴 SOTTO 0G (Short Bias)"
                if abs(px - sd1_down) / px < 0.005: status_label = "🔴 TEST -1SD (Supporto?)"
            else:
                status_label = "🟢 SOPRA 0G (Long Bias)"
                if abs(px - sd1_up) / px < 0.005: status_label = "🟡 TEST +1SD (Rottura?)"
            
            if abs(dist_zg_pct) < 0.3: status_label = "🔥 FLIP IMMINENTE"
    ...
    status_text.empty()
    if scan_results:
        final_df = pd.DataFrame(scan_results).sort_values("_sort")
        st.dataframe(final_df.drop(columns=["_sort"]), use_container_width=True, height=800)
