...
try:
    ...
    with open("hist_info.txt", "w") as f:
        f.write(str(dir(histdatacom)) + "\n")
        f.write(str(type(histdatacom)) + "\n")
...
def fetch_data_smart(ticker, timeframe, start_date, end_date):
    ...
    if is_forex:
        try:
            ...
            # Esecuzione download
            if hasattr(histdatacom, 'HistDataCom'):
                histdatacom.HistDataCom(options).run()
            else:
            ...
            if not zip_files:
                raise Exception("Nessun file ZIP scaricato da HistData.")
            else:
                for zf in zip_files:
                ...
                if csv_files:
                    ...
                    raise Exception("Nessun file CSV trovato negli archivi estratti.")
        except Exception as e:
            st.warning(f"⚠️ Errore HistData ({e}), tentativo con Yahoo Finance...")
            try:
                tf_yf = "1d"
                if timeframe in ["1m", "1Min"]: tf_yf = "1m"
                elif timeframe in ["5m", "5Min"]: tf_yf = "5m"
                elif timeframe in ["15m", "15Min"]: tf_yf = "15m"
                elif timeframe in ["1h", "1H"]: tf_yf = "1h"
                
                t = yf.Ticker(f"{clean_ticker}=X")
                df_yf = t.history(start=start_date.strftime('%Y-%m-%d'), end=end_date.strftime('%Y-%m-%d'), interval=tf_yf)
                if not df_yf.empty:
                    df_yf.reset_index(inplace=True)
                    df_yf.rename(columns={'Date': 'datetime', 'Datetime': 'datetime'}, inplace=True)
                    df = process_dataframe(df_yf, ticker, start_date, end_date)
                    st.success(f"✅ Forex: {ticker} caricato con successo da Yahoo Finance ({len(df)} righe).")
                else:
                    st.error(f"❌ Errore critico Forex: Dati non trovati su Yahoo Finance.")
            except Exception as e2:
                st.error(f"❌ Errore critico Forex: {e} e fallback fallito ({e2}).")
            ...
            try:
                ...
                try:
                    ...
                    if not df_yf.empty:
                        ...
                        st.error(f"Errore Forex: Dati non trovati su Yahoo Finance.")
                except Exception as e2:
                    st.error(f"Errore Forex: {e} e fallback fallito ({e2}).")
        else:
            try:
                tf_alpaca = timeframe
