...
if menu == "🏟️ DASHBOARD SINGOLA":
    ...
    if selected_dte:
        ...
        if not raw_data.empty:
            ...
            # --- MODIFICA CRUCIALE: NESSUN BINNING ---
            # Uso direttamente visible_agg che contiene gli strike puri
            p_df = visible_agg.copy()
            
            # Nessuna aggregazione per 'bin'. Disegno ogni singolo strike.
            
            fig = go.Figure()
            # Uso p_df['strike'] invece di un bin
            fig.add_trace(go.Bar(y=p_df['strike'], x=p_df[metric], orientation='h',
                                 ...
                                 # Width dinamico leggero per far vedere le linee sottili
                                 width=None)) 
            ...
            fig.update_layout(template="plotly_dark", height=800, margin=dict(l=0,r=0,t=0,b=0),
                              yaxis=dict(range=[lo, hi], gridcolor="#333", title="Strike Price"), # Tolto dtick fisso per lasciare a Plotly la gestione dei mille strike
